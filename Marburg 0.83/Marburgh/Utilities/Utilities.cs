...
public class Utilities
{
    ...
    internal static int check(string v)
    {
        return ((int)(Console.WindowWidth / 2) - (v.Length / 2));
    }

    internal static void CombatText(string v1, string v2, string v3)
    {        
    ...
    {
        death++;
        Combat.currentAttackOptions = new string[] { "", "", "", "" };
        Console.Clear();
        if (death > 2)
        {
            Console.Clear();
    ...
    public static void ResetNormalRooms()
    {
        ...
        {
            bool reset = true;
            for (int x = 0; x < Marburgh.Program.d.specialRoomOptions.Length; x++)
            {
                if (Marburgh.Program.d.shell[i].assignedRoom == Marburgh.Program.d.specialRoomOptions[x]) reset = false;
            }

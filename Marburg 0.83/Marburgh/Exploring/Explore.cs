...
class Explore
{
   ...
   public static void GameDungeon(Dungeon d, Creature p)
   {
        ...
        Console.Clear();              
        for (int i = 0; i < Shell.StarterDungeonShell.Length; i++)
        {
            if (Shell.StarterDungeonShell[i].current) currentShell = Shell.StarterDungeonShell[i];
        }
    ...
    private static void RoomSelection(Dungeon d , Creature p)
    {
        ...
        //If not, move on
        bool special = true;
        for (int i = 0; i < d.roomOptions.Length; i++)
        {
            if (currentShell.assignedRoom.name == d.roomOptions[i].name)
            {
                special = false;
                break;
            }
        }
        if (special == true) Room.SpecialRoom(d, p, currentShell.assignedRoom.EventArray);
